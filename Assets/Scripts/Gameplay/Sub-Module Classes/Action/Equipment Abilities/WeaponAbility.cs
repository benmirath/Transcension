...
public abstract class WeaponAbility : BaseAction {	
	#region Fields
	//Inspector Fields
...
/*	/// <summary>
	...
	protected virtual void OnTriggerEnter (Collider hit) {										//The effect of the attack when a potential target enters the attack hitbox		
		if (hit.GetComponent<BaseEnemy>()) {
			ICharacter target = hit.GetComponent<BaseEnemy>();
			Vector3 hitDir;
			
			if (target.CharState.IsEvading) return;														//checks to see if the attack is evaded, cancels effect of attack
			
			if (target.CharState.IsDefending) {																//checks to see if the attack is blocked
				if(Vector3.Dot(target.CharPhysics.Coordinates.up, _curWeapon.User.CharPhysics.Coordinates.position) > 0.75f) {
					Debug.Log ("Attack blocked!");
					hitDir = (target.CharPhysics.Coordinates.position - _curWeapon.User.CharPhysics.Coordinates.position).normalized;
					
					StopCoroutine ("Activate");
					Char.CharActions.CharCombat.ApplyHitStun(stats.AttackStrength, hitDir);
				}
			}

			hitDir = (target.CharPhysics.Coordinates.position - _curWeapon.User.CharPhysics.Coordinates.position).normalized;			
			//target.CharActions.ApplyDamage(_curWeapon.Stats.AdjustedBaseDamage);
			Debug.LogError ("the attack strength of the attack is :" + stats.AttackStrength);
			target.CharActions.CharCombat.ApplyHitStun(stats.AttackStrength, hitDir);
	...
	public abstract class StandardAttack : WeaponAbility
	{
		...
		public class FinisherAttack : StandardAttack
		{
			...
			{
				Char.CharPhysics.Controller.Move(Char.CharPhysics.Coordinates.up * Stats.Speed * Time.deltaTime);
			}
			//turn off followup
		}
	...
	[System.Serializable] public class AttackStats : BaseAction.AbilityStats {
		#region Fields
